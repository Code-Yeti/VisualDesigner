...
function bakeAnimationState(svg: SVGSVGElement, timeSeconds: number): void {
  ...
  animatedEls.forEach((el) => {
    ...
    // Each element carries its own `data-dash-repeat` (see
    // computeDashRepeatLength) - using a single constant here would
    // reintroduce the same seam-jump bug the per-element keyframes fix: it
    // only looks seamless when this exactly matches that element's actual
    // dash+gap repeat length.
    const dashRepeat = parseFloat(el.getAttribute("data-dash-repeat") ?? "19");
